...
public abstract class AbstractLogger {
	...
	public HttpHeaders getHttpHeaders(HttpServletRequest e) {
    	HttpHeaders headers = new HttpHeaders();
    	Collections.list(e.getHeaderNames()).forEach(name -> {
    		//If needed we can filter only required headers
    		headers.put(name, Collections.list(e.getHeaders(name)));
    	});
		return headers;
	}
